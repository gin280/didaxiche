<template>
  <div class="dashboard-container">
    <div class="dashboard-management">
      <div class="dashboard-text">修改优惠卷</div>
    </div>
    <div class="user">
        ...
        <el-tab-pane label="基本信息" name="information">
          <div class="information" style="border-bottom: 1px solid #CFCDCD;">
              ...
              <div class="information-basic-serial">
                ...
                <div class="information-basic-serial">
                  ...
                  <div class="information-basic-serial">
                    ...
                    <div class="information-description">优惠券总数量，没有不能领取或发放,-1 为不限制张数</div>
                  </div>
                </div>
            ...
            <div class="review-wen">
              <div class="review-letf" type="primary" @click="basicT = true">确认修改</div>
        ...
        <el-tab-pane label="领取设置" name="receive">
          <div class="information-basic">
            <div class="information-basic-serial">
              ...
              <div class="information-serial">
                <div class="information-serial-title marginTop">加入领劵中心</div>
            ...
            <el-button type="primary" class="submibtn" @click="basicT = true">提交按钮</el-button>
          </div>
        </el-tab-pane>
            ...
            <div class="information-basic">
              ...
              <div class="information-basic-serial">
                <div class="information-serial">
                  ...
                  <div class="information-serial-input">
                    <input v-model="templateimg" style="width:270px;" disabled>
                  </div>
                  <el-button class="but">点击上传</el-button>
                </div>
                <div class="upload">
                  <!-- <el-upload :action="Url" list-type="picture-card" :on-preview="PreviewLicense"
...
export default {
    ...
    data() {
        return {
            ...
            activityKeyword: '', // 关键字
            picturesG: '', // 图片组
            dialogVisibleSelect: false, // 选择网点类型的对话框
            ruleForm: {
                total_sum: ''
            },
    ...
    mounted() {
        ...
        //获取优惠劵分类
        getClassificationCoupon({ 'id': this.id }).then(res => {
            // console.log(res.result)
            this.classification = res.result
        })
        //获取优惠劵详情
        getCoupon({ 'id': this.id }).then(res => {
            // console.log(res);
            this.id = res.result.id
            this.name = res.result.name
            this.sort = res.result.sort
            this.classification.forEach(val => {
                if (val.id == res.result.cid) {
                    this.cid.id = val.id
                    this.cid.name = val.name
                }
            })
            console.log(this.cid)
            this.money_rule = res.result.money_rule
            this.money_reduce = res.result.money_reduce
            this.month_type = res.result.month_type
            if (res.result.month_type == 1) {
                //获取后的时间
                this.month = res.result.month
            } else if (res.result.month_type == 2) {
                //时间范围
                this.Data = [res.result.month_start, res.result.month_end]
            }
            this.number = res.result.number
            this.type = res.result.type
            this.share_type = res.result.share_type
            this.total_sum = res.result.share[0]
            this.highest_receive = res.result.share[1]
            this.minimum_receive = res.result.share[2]
            this.most_number = res.result.share[3]
            this.jia_status = res.result.jia_status
            // 推送
            this.templateid = res.result.templateid
            this.templatetitle = res.result.templatetitle
            // 推送封面
            if (res.result.templateimg !== '') {
                this.templateimg = res.result.templateimg
                this.licenseImgS.push({ url: this.templateimg })
            }
            this.templateremarks = res.result.templateremarks
            this.templateurl = res.result.templateurl
            this.remarks = res.result.remarks  // 使用说明
        }) /
      // 富文本初始化
      tinymce.init({
          selector: '#tinymce'
    ...
    // 优惠类型变化的监听
        ...
        //     this.placeholders.city = data.city.value
        //     this.placeholders.area = data.area.value
        //     this.placeholders.provinces = data.provinces.value
        // },
        // 图片上传成功
        upSuccess(response) {
