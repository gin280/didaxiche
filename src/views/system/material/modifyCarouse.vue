<template>
  <div class="dashboard-container">
    <div class="dashboard-management">
      <div class="dashboard-text">修改轮播图</div>
    </div>
    <div class="user">
      ...
      <div class="save" @click="add">修改轮播图</div>
    </div>
...
<script>
import { modifyCarousel, getCarousel } from '@/api/system.js'
...
export default {
  ...
  methods: {
    ...
    get() {
      getCarousel({ 'id': sessionStorage.getItem('id') }).then(res => {
        this.numberInput = res.result.serial
        this.titleInput = res.result.title
        this.pictureGroup.push({ 'url': res.result.image })
        this.imgInput = res.result.image
        this.InsuranceId = res.result.s_id
        this.linkInput = res.result.image_url
        this.radio = res.result.status
      })
    },
    add() {
      if (this.imgInput === '') {
