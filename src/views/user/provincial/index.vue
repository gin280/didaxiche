<template>
  <div class="dashboard-container">
    <div class="dashboard-management">
      <div class="dashboard-text">省级代理商管理</div>
    ...
    <div class="user">
      <div class="user-number">省级代理商总数:{{ merchants }}个</div>
      <el-row class="user-head" type="flex" align="middle">
        ...
        <el-col :span="3">市代数量</el-col>
        <el-col :span="4">设备数量</el-col>
        <el-col :span="4">网点数量</el-col>
      ...
      <el-row v-for="(item,index) in list" :key="index" class="user-body" type="flex" align="middle">
        ...
        <el-col :span="3">{{ item.agentcount }}</el-col>
        <el-col :span="4">{{ item.cmdcount }}</el-col>
        <el-col :span="4" class="user-body-time">{{ item.bcount }}</el-col>
        ...
        <el-col :span="4" class="user-body-operating">
          <svg-icon icon-class="jinzhi" @click="ProvinceStatus(item.id,item.enable)" />
    ...
    <div class="edit">
      <!-- <div class="edit-detele" @click="confirmDeletion">批量删除</div> -->
      <!-- <div class="addEquipment" @click="Add">新增省级代理</div> -->
      <div class="edit-detele" style="cursor:pointer" @click="Add">新增省级代理</div>
...
<script>
import {
    getProvincialAgent, // 省级代理商管理
    deleteProvince,  // 删除
    setProvinceStatus  //状态
} from '@/api/user.js'
export default {
    data() {
        return {
    ...
    // 跳转
        goDetails(id) {
            this.$router.push({ path: '/user/provincial/details', query: { id: id }})
        },
        // 添加省代
        Add(id) {
            this.$router.push({ path: '/user/provincial/AddAgent'})
        },
        // 获取省级列表
        getList(pages) {
            getProvincialAgent({ 'name': this.searchInput, 'pages': pages }).then(res => {
        ...
        ProvinceStatus(id, enable) {
            ...
            // 更改状态
            setProvinceStatus({ 'id': id, 'enable': this.status, 'aid': sessionStorage.getItem('admin_id') }).then(res => {
                if (this.status === '0') {
                    this.$message({
                        message: '已启用',
                        type: 'success'
        ...
        // 删除省代理商
        Udelete(id) {
            deleteProvince({ 'id': this.id }).then(res => {
                this.$message({
                    message: '删除成功',
                    type: 'success'
                })
