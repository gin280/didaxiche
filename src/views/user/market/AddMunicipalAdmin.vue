<template>
  <div class="dashboard-container">
    <div class="dashboard-management">
      <div class="dashboard-text">新增市代管理员</div>
    </div>
    <div class="user">
      ...
      <div class="add-flex">
        ...
        <el-input v-model="passwordInput" autocomplete="new-password" class="add-w500" placeholder="请设置密码" show-password />
      </div>
      <div class="add-flex">
        <div class="add-p6">重复密码</div>
        <el-input v-model="passwordTowInput" class="add-w500" placeholder="请再次输入密码" show-password />
      </div>
      <div class="save" @click="addBtn">确认新增</div>
    </div>
  </div>
</template>

<script>
import { getaccountManagementAdd } from '@/api/user.js'
export default {
    data() {
        return {
            id: '',
    ...
    methods: {
        ...
        // 新增市代管理员
        addBtn() {
            if (this.nameInput === '' || this.phoneInput === '' || this.accountInput === '' || this.passwordInput === '' || this.passwordTowInput === '') {
                this.$message({
                    message: '必填项不能为空',
                    type: 'warning'
                })
            ...
            getaccountManagementAdd({
                'id': this.id,
                'realname': this.nameInput,
                'phone': this.phoneInput,
                'username': this.accountInput,
                'password_hash': this.passwordInput,
                'password_hashtwice': this.passwordTowInput
            }).then(res => {
                this.$message({
                    message: '新增市代管理员账号成功',
                    type: 'success'
